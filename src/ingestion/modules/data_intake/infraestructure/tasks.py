from seedwork.infraestructure.celery import app
import uuid
...
@app.task
def start_ingestion_task(ingestion_uuid: str, coreography_id: str):
    """
    ...
    ProcessIngestionService().process_ingestion(
        uuid.UUID(ingestion_uuid), uuid.UUID(coreography_id)
    )
