...
class SQLAlchemyRepository(Repository, Generic[EnttyDTOType, EntityType]):
    ...
    def add(self, data_canonization: EntityType):
        enity_dto: EnttyDTOType = self.entity_factory.create_object(
            data_canonization, self.entity_mapper
        )
        db.add(enity_dto)

    def update(self, data_canonization: EntityType):
        db.add(data_canonization)

    def delete(self, data_canonization: EntityType):
        db.delete(data_canonization)
